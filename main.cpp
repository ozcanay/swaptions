...
static void setLogger() {
  try {
    const auto logger =
        spdlog::basic_logger_mt("config_manager", "logs/swaptions.txt", true);
  ...
  void operator()(const tbb::blocked_range<int> &range) const {
    ...
    for (int i = begin; i != end; i++) {
      int iSuccess = HJM_Swaption_Blocking(
          ...
          swaptions[i].dMaturity, swaptions[i].dTenor,
          swaptions[i].dPaymentInterval, swaptions[i].iN, swaptions[i].iFactors,
          swaptions[i].dYears, swaptions[i].pdYield, swaptions[i].ppdFactors,
...
#define __PARSEC_XSTRING(x) __PARSEC_STRING(x)
  ...
  printf("Number of Simulations: %d,  Number of threads: %d Number of "
         "swaptions: %d\n",
  ...
  for (i = 0; i < nSwaptions; i++) {
    ...
    swaptions[i].ppdFactors =
        dmatrix(0, swaptions[i].iFactors - 1, 0, swaptions[i].iN - 2);
