// HJM_Securities.cpp
// Routines to compute various security prices using HJM framework (via
// Simulation). Authors: Mark Broadie, Jatin Dewanwala Collaborator: Mikhail
// Smelyanskiy, Jike Chong, Intel Modified by Christian Bienia for the PARSEC
// Benchmark Suite
...
static void setLogger() {
  try {
    const auto logger = spdlog::basic_logger_mt("swaptions", "logs/swaptions.txt", true);
    spdlog::set_pattern("[%T:%F][%l][%s:%#] %v");
...
long seed = 1979;  // arbitrary (but constant) default value (birth year of
                   // Christian Bienia)
long swaption_seed;
...
struct Worker {
  ...
  void operator()(const tbb::blocked_range<int> &range) const {
    ...
    for (int i = begin; i != end; i++) {
      int iSuccess = HJM_Swaption_Blocking(pdSwaptionPrice, swaptions[i].dStrike, swaptions[i].dCompounding, swaptions[i].dMaturity, swaptions[i].dTenor, swaptions[i].dPaymentInterval, swaptions[i].iN, swaptions[i].iFactors, swaptions[i].dYears, swaptions[i].pdYield, swaptions[i].ppdFactors,
...
void *worker(void *arg) {
  ...
  if (tid == nThreads - 1) end = nSwaptions;

  SPDLOG_INFO("tid: {}, beg: {}, end: {}, chunksize: {}", tid, beg, end, chunksize);
  ...
  SPDLOG_INFO("THREAD ENDED tid: {}", tid);
...
// Please note: Whenever we type-cast to (int), we add 0.5 to ensure that the
// value is rounded to the correct number. For instance, if X/Y = 0.999 then
// (int) (X/Y) will equal 0 and not 1 (as (int) rounds down). Adding 0.5 ensures
// that this does not happen. Therefore we use (int) (X/Y + 0.5); instead of
// (int) (X/Y);
...
#define __PARSEC_XSTRING(x) __PARSEC_STRING(x)
  ...
  printf(
      "Number of Simulations: %d,  Number of threads: %d Number of "
      "swaptions: %d\n",
  ...
  for (i = 0; i < nSwaptions; i++) {
    ...
    for (j = 1; j <= swaptions[i].iN - 1; ++j) swaptions[i].pdYield[j] = swaptions[i].pdYield[j - 1] + .005;

    swaptions[i].ppdFactors = dmatrix(0, swaptions[i].iFactors - 1, 0, swaptions[i].iN - 2);
    for (k = 0; k <= swaptions[i].iFactors - 1; ++k)
      for (j = 0; j <= swaptions[i].iN - 2; ++j) swaptions[i].ppdFactors[k][j] = factors[k][j];
  }
